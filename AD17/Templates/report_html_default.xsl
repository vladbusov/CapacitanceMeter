...
<xsl:template name="process_body">
	<img ALT="Altium">
		<xsl:attribute name="src">
			file://<xsl:value-of select="//resource_path"/>AD_logo.png
		</xsl:attribute>
	</img> 
...
<xsl:template match="table">
	...
	<xsl:if test="columns">
		...
		<xsl:variable name="hasSecondColumn">
			<xsl:choose>
				<xsl:when test="columns/column/subcolumns">1</xsl:when>
		...
		<xsl:element name="table">		
		  			...
		  			<xsl:otherwise>
		  				<xsl:for-each select="columns/column" >
		  					<xsl:element name="th">             
								...
								<xsl:choose>
									<xsl:when test="$hasSecondColumn=1">
										<xsl:if test="count(subcolumns/column) = 0">
											<xsl:attribute name="rowspan">2</xsl:attribute>
										</xsl:if>

										<xsl:if test="count(subcolumns/column) > 0">
											<xsl:attribute name="colspan">
												<xsl:value-of select="count(subcolumns/column)"/>
											</xsl:attribute>
										</xsl:if>
			...
			<!--start the child column-->
				<xsl:if test="columns">
					<xsl:for-each select="columns/column" >
						<xsl:if test="subcolumns">
							<xsl:variable name="column_position" select="position()"/>
							<xsl:element name="tr">
								<xsl:for-each select="subcolumns/column" >
									<xsl:variable name="child_column_position" select="position()"/>
									<xsl:element name="th">
										<xsl:attribute name="class">
											<xsl:value-of select="concat('column',$column_position + $child_column_position - 1)"/>
										</xsl:attribute>
										<xsl:apply-templates select="."/>
									</xsl:element>
								</xsl:for-each>
				...
				<tr>
					...
					<xsl:for-each select="columns/column">
						...
						<xsl:if test="not($column_position = 1)">
							<xsl:choose>
								<xsl:when test="subcolumns">
									<xsl:for-each select="subcolumns/column">
										<xsl:choose>
											<xsl:when test="@data_type='number'">
												<xsl:variable name="child_column_position" select="position()"/>
												<td style="font-weight : bold">
													<xsl:attribute name="class">
														<xsl:value-of select="concat('column',$column_position + $child_column_position - 1)"/>
													</xsl:attribute>
													<xsl:value-of select="sum(ancestor::table/row/cell[$column_position + $child_column_position - 1])"/>
												</td>
											</xsl:when>
											<xsl:otherwise>
												<td/>
